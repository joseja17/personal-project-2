...
setInterval(function() { const twilioDB = app.get('db');
twilioDB.get_all_schedules().then(resp => {
    console.log(resp);
    console.log(new Date());
    for (var i = 0; i < resp.length; i++){
        var strEventDate = resp[i].event_date.split('-');
        var dateString = new Date();
        var day = dateString.getDate();
        var month = dateString.getMonth()+1;
        var year = dateString.getFullYear();
        var monthToNum = (parseInt(strEventDate[1]));
        var dateToNum = (parseInt(strEventDate[2]));
        console.log(new Date());
        if (strEventDate[0] == year && monthToNum == month && dateToNum - day <= 1) {
            client.messages.create({
                from: process.env.TWILIO_PHONE_NUMBER,
                to: '4357737283',
                body: "Team Event Coming Up! Check Website"
