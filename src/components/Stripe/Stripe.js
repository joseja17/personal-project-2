    ...
    return (
      <div>
        ...
        <Tablet>
          ...
          <div className="Stripe-tablet">
            <ToastContainer autoClose={3400} />

            <div>
              <Header />
            </div>
            <div className='stripe-box-tablet'>
              <div className="Stripe-header-tablet">
                {/* <img src='/logo.svg' className="Stripe-logo" alt="logo" /> */}
                <h2 className='h2-stripe-tablet'>Donate with Stripe</h2>
                <select className='stripe-inputt-tablet' onChange={e => this.handleOnSelect(e)} placeholder=' Select Team'><option>Select Team</option>{teamNameToDisplay}</select>
                <input className='stripe-input-tablet' type='number' placeholder=' Amount' onChange={(e) => this.changeAmount(e.target.value)} />
              </div>
              <StripeCheckout className="sc"
                description={this.state.description}
                token={this.onToken} // get token back
